...
def simulation_list_manual_insert(name:Optional[List[str]] = None,
                                  _fsim = FortranSimulationManualInsertInitialCondition()) -> List[Itens]:
    """
    Dada um nome ou uma lista de nomes, essa função verifica se os mesmos está(ão) contidos 
    em fortran_simulation_manual_insert_initial_condition.json, caso esteja uma lista com os objetos 
    com nomes da simulações é retornado, caso não esteja um erro é levantado. 
